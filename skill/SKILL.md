...
2. **配置已就绪？**
   ...
   `qiniu-logs config` 会脱敏只显示 AK/SK 前 4 位；看到 access_key / secret_key / bucket / domain 非空且 `默认项目:` 有值即就绪。若报 `配置错误: X 不能为空`，回到 2.1 补齐相应字段。
...
注意：
- 首行 `找到 N 个日志文件（项目: X）` 给出**总数**，可 `head -1 | grep -oE '[0-9]+'` 抽取（项目名一般不含数字，`grep -oE '[0-9]+' | head -1` 仍可取总数；若项目名含数字改用更精确的提取）
...
- 空结果输出 `未找到用户 <uid> 的日志文件（项目: X）`，据此向用户回退（uid 错？时间窗太窄？项目选错？）
